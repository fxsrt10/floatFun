...
namespace Leap.Interact
{
		...
		public class LeapInteraction : MonoBehaviour
		{
				...
				[HideInInspector]
				public bool
						EnableCollisionShapeHolding = false;
				[HideInInspector]
				public bool
						EnableAnchorBasedHolding = true;

				// Use palm's positon to position the held body.
				[HideInInspector]
				public bool
						PositionFromPalm = false;
				// Use palm's rotation to orient the held body.
				[HideInInspector]
				public bool
						RotationFromPalm = true;
				...
				[HideInInspector]
				public bool
						EnableClicking = true; // not exposed
				...
				[HideInInspector]
				public bool
						GenerateDefaultClickAnchors = false;

				// Always enable smooth grabbing
				[HideInInspector]
				public bool
						EnableSmoothGrabbing = true;


				// Rotational constraint is only going to be enforced at this strength (0-1 scale).
				[HideInInspector]
				public float
						MinStrengthToLockRotation = 0.0f;
				[HideInInspector]
				public bool
						CollisionsWithHandFilteredOut = false;
				[HideInInspector]
				public bool
						isGrabbed = false;
				...
				[HideInInspector]
				public Vector3
						tmpAngularVelocity = Vector3.zero;
				[HideInInspector]
				public bool
						velocityToTransfer = false;
				[HideInInspector]
				public float
						scale = 1.0f;
				...
				public void ApplyToBody (Body body)
				{
						...
						if (GenerateAnchors) {
								body.GenerateDefaultAnchors ();
						}
						if (GenerateDefaultClickAnchors) {
								body.GenerateDefaultClickAnchors ();
						}
						if (EnableSmoothGrabbing) {
								body.EnableSmoothGrabbing ();
						}
