...
public class InteractionSceneSetup : MonoBehaviour
{
		...
		[HideInInspector]
		public float
				clientUnitLengthInMillimeters = 50f;
		...
		[HideInInspector]
		public Vector3
				OffsetCameraToLeap = new Vector3 (0, 0, 0);
		...
		[HideInInspector]
		public bool
				ShowInteractionHands = false;
		[HideInInspector]
		public bool
				ShowVisualizer = false; // disabled flag
		[HideInInspector]
		public bool
				EnableInteractionCollisions = false;
		...
		public void DisableHandCollisions (Holding holding)
		{
				...
				if (body != null && body.IsValid ()) {
						...
						game_object.GetComponent<LeapInteraction> ().OnGrab ();
				}
		...
		private void EnableHandCollisions (Holding holding)
		{
				...
				if (body != null && body.IsValid ()) {
						game_object = UnityUtil.BodyMapper.FirstOrDefault (x => x.Value.BodyId.ptr == body.BodyId.ptr).Key;
						game_object.GetComponent<LeapInteraction> ().OnRelease ();
				}
